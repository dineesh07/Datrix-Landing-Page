...
export default function PricingSection() {
  ...
  return (
    <section 
      ...
      className="py-24 bg-arctic-powder relative overflow-hidden"
    >
      {/* Decorative gradients */}
      <div className="absolute top-1/3 left-1/4 w-[300px] h-[300px] bg-forsythia/10 rounded-full blur-[80px] pointer-events-none" />
      <div className="absolute bottom-1/3 right-1/4 w-[300px] h-[300px] bg-deep-saffron/10 rounded-full blur-[80px] pointer-events-none" />
      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="flex flex-col items-center text-center mb-16">
          <h2 
            id="pricing-heading" 
            className="text-3xl sm:text-4xl font-bold font-heading text-nocturnal mb-4"
          >
            One Platform. Every Scale.
          </h2>
          <p className="text-base text-nocturnal/70 max-w-xl mx-auto font-body mb-10">
          ...
          <div className="flex flex-col sm:flex-row items-center gap-6 z-10">
            ...
            <div 
              ...
              className="relative flex p-1 bg-mystic-mint/30 rounded-xl border border-mystic-mint/80 h-12 w-64 items-center"
            >
              {/* Animated Slide Indicator */}
              <div 
                className="absolute top-1 bottom-1 left-1 bg-nocturnal rounded-lg shadow-sm transition-all duration-150 ease-out"
                style={{
              ...
              {(['INR', 'USD', 'EUR'] as Currency[]).map((c) => (
                <button
                  ...
                  className={`relative z-10 w-1/3 text-xs font-semibold font-heading h-full rounded-lg transition-colors duration-150 ${
                    currency === c ? 'text-arctic-powder' : 'text-nocturnal/75 hover:text-nocturnal'
                  }`}
                >
                  {c}
                </button>
            ...
            <div 
              ...
              className="flex items-center gap-3 cursor-pointer select-none bg-mystic-mint/10 border border-mystic-mint/40 py-2 px-4 rounded-xl hover:bg-mystic-mint/20 transition-colors duration-micro"
            >
              <span className={`text-xs font-semibold font-body ${!isAnnual ? 'text-nocturnal' : 'text-nocturnal/60'}`}>
                Monthly
              </span>
              
              {/* Toggle Track */}
              <div className="relative w-10 h-6 bg-mystic-mint rounded-full transition-colors duration-layout">
                {/* Thumb */}
                <div 
                  className="absolute top-1 left-1 w-4 h-4 bg-nocturnal rounded-full transition-transform duration-layout"
                  style={{
              ...
              <span className={`text-xs font-semibold font-body flex items-center gap-1.5 ${isAnnual ? 'text-nocturnal' : 'text-nocturnal/60'}`}>
                Annual 
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-6xl mx-auto items-stretch mb-12">
          {PRICING_TIERS.map((tier) => {
            ...
            return (
              <div
                key={tier.id}
                className={`relative flex flex-col justify-between p-8 rounded-2xl border transition-all duration-micro bg-arctic-powder ${
                  tier.highlight 
                    ? 'border-deep-saffron shadow-xl scale-[1.03] z-10' 
                    : 'border-mystic-mint hover:border-nocturnal/20'
                }`}
              >
                ...
                <div>
                  <h3 className="text-xl font-bold font-heading text-nocturnal mb-2">
                    {tier.name}
                  </h3>
                  <p className="text-xs text-nocturnal/65 font-body min-h-[32px]">
                    {tier.tagline}
                  </p>
                  ...
                  <div className="my-8 flex items-baseline gap-2">
                    <span 
                      ...
                      className="text-4xl sm:text-5xl font-bold font-heading text-nocturnal tracking-tight"
                    >
                      {initialPrice.display}
                    </span>
                    <span className="text-sm font-medium text-nocturnal/50 font-body">
                      / month
                    </span>
                  </div>

                  {/* Feature Checklist */}
                  <ul role="list" className="space-y-4 border-t border-mystic-mint/30 pt-6">
                    {tier.features.map((feature, index) => (
                      <li key={index} className="flex items-start gap-3">
                        ...
                        <span className="text-sm text-nocturnal/80 font-body leading-tight">
                          {feature}
                        </span>
                      </li>
                    ))}
                ...
                <div className="mt-10">
                  <a
                    ...
                    className={`block w-full py-4 rounded-xl text-center font-semibold text-sm transition-all duration-micro ${
                      tier.highlight
                        ? 'btn-primary'
                        : 'btn-secondary border border-mystic-mint/80 hover:border-nocturnal'
                    }`}
                  >
                    {tier.cta}
                  </a>
        ...
        {/* Bottom Trust Signal */}
        <p className="text-center text-xs text-nocturnal/55 font-body mt-8">
